// ==UserScript==
// @name         Rem & Ram + Senko San (Chatbot Final) - Versiune 8.1
// @namespace    http://tampermonkey.net/
// @version      8.1
// @description  Chatbot complet cu Rem, Ram și Senko San: 3 moduri, comenzi, culori personalizabile, q1 aplicat global, resetare modul la refresh, excepții stricte pentru qq/qw/qe/sterge mesaje, emoji picker îmbunătățit cu inger și scroll automat.
...
(function() {
    ...
    // Culori implicite (opacitate solidă)
    let COLORS = {
    ...
    let angelEmojiPicker, colorPicker, fileModal, helpModal, modeModal;

    // ============================================
    // 🎨 EMOJI-URI EXTINSE (NOI)
    // ============================================
    const ANGEL_EMOJIS = [
        // Îngeri și emoji-uri legate de îngeri
        "😇", "👼", "🧚", "✨", "🌟", "💫", "🌈", "🌠", "🎆", "🎇",
        // Emoji-uri de bază
        "😊", "😍", "😂", "😢", "😘", "😭", "😱", "😎", "🤔", "🤗", "🤣", "😴", "😇", "🙏", "❤️", "💖", "💘", "💝", "💔", "🔥",
        // Culori
            ...
            sections: [
                {
                    ...
                    content: [
                        ...
                        "• <b>q1</b> - Activează/Dezactivează modul restrâns (blochează toate comenzile EXCEPTÂND qq, qw, qe, sterge mesaje).",
                ...
                },
                {
                    ...
                    content: [
                        ...
                        "• <b>Buton 👼</b> - Selectează emoji-uri.",
    ...
    function regenerateCSS() {
        ...
        style.textContent = `
            ...
            .dual-chatbot-angel-emoji-button {
            ...
            #dual-chatbot-angel-emoji-picker {
                position: fixed;
                bottom: 70px;
                right: 20px;
                background: white;
            ...
            #dual-chatbot-angel-emoji-picker.active {
                display: block;
            }

            .dual-chatbot-angel-emoji-grid {
                display: grid;
            ...
            .dual-chatbot-angel-emoji-option {
                display: flex;
                align-items: center;
                justify-content: center;
                font-size: 1.4em;
            ...
            .dual-chatbot-angel-emoji-option:hover {
                background: #f0f0f0;
            }
    ...
    function handleNormalCommand(command) {
        ...
        // Excepții stricte (funcționează și cu q1 activat)
        ...
        if (cmd === "qq" || cmd.startsWith("qq ")) {
            const msg = cmd.startsWith("qq ") ? cmd.substring(3) : "";
            if (msg) addMessage(REM_NAME, msg);
            return true;
        }

        if (cmd === "qw" || cmd.startsWith("qw ")) {
            const msg = cmd.startsWith("qw ") ? cmd.substring(3) : "";
            if (msg) addMessage(RAM_NAME, msg);
            return true;
        }

        // Comenzi blocate dacă q1 este activat
        if (q1Mode) return false;
    ...
    function handleAllModeCommand(command) {
        ...
        if (cmd === "qe" || cmd.startsWith("qe ")) {
            const msg = cmd.startsWith("qe ") ? cmd.substring(3) : "";
            if (msg) addMessage(SENKO_NAME, msg);
            return true;
        }
    ...
    function createDOM() {
        ...
        const angelEmojiButton = document.createElement('button');
        angelEmojiButton.className = 'dual-chatbot-button dual-chatbot-angel-emoji-button';
        angelEmojiButton.textContent = '👼';
        angelEmojiButton.title = 'Selectează emoji';
        ...
        inputPanel.appendChild(angelEmojiButton);
        ...
        // --- ANGEL EMOJI PICKER ---
        angelEmojiPicker = document.createElement('div');
        angelEmojiPicker.id = 'dual-chatbot-angel-emoji-picker';

        const emojiGrid = document.createElement('div');
        emojiGrid.className = 'dual-chatbot-angel-emoji-grid';

        ANGEL_EMOJIS.forEach(emoji => {
            const emojiOption = document.createElement('span');
            emojiOption.className = 'dual-chatbot-angel-emoji-option';
            emojiOption.textContent = emoji;
            emojiOption.addEventListener('click', () => {
                messageInput.value += emoji;
                messageInput.focus();
                angelEmojiPicker.classList.remove('active');
            });
            emojiGrid.appendChild(emojiOption);
        });

        angelEmojiPicker.appendChild(emojiGrid);
        ...
        document.body.appendChild(angelEmojiPicker);
    ...
    function updateHeaderAvatars(headerLeft) {
        ...
        avatarsToShow.forEach(name => {
            ...
            avatar.src = selectedAvatars[name] + '?v=' + Date.now();
            avatar.alt = name;
    ...
    function renderMessages() {
        ...
        messages.forEach((msg) => {
            ...
            avatarImg.src = selectedAvatars[msg.sender] + '?v=' + Date.now();
    ...
    function init() {
        ...
        elements.sendButton.addEventListener('click', () => {
            ...
            if (message) {
                const isCommand = message.startsWith("qq ") || message.startsWith("qw ") || message.startsWith("qe ") || message === "qq" || message === "qw" || message === "qe";
                if (!isCommand) {
        ...
        // Angel Emoji Picker
        elements.angelEmojiButton.addEventListener('click', () => {
            angelEmojiPicker.classList.toggle('active');
        });
        ...
        document.addEventListener('click', (e) => {
            if (!angelEmojiPicker.contains(e.target) && e.target !== elements.angelEmojiButton)
                angelEmojiPicker.classList.remove('active');
