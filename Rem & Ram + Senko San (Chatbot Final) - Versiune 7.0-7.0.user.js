// ==UserScript==
// @name         Rem & Ram + Senko San (Chatbot Final) - Versiune 7.0
// @namespace    http://tampermonkey.net/
// @version      7.0
// @description  Chatbot complet cu Rem, Ram și Senko San: 3 moduri, comenzi, culori personalizabile, opțiuni transparență mesaje, culori nume individuale. User: mov/alb. 100% funcțional.
...
(function() {
    ...
    const DEFAULT_AVATARS = {
        [USER_NAME]: "https://upload.wikimedia.org/wikipedia/commons/thumb/7/7c/Profile_avatar_placeholder_large.png/200px-Profile_avatar_placeholder_large.png",
        [REM_NAME]: "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5e/Re_Zero_Rem.png/200px-Re_Zero_Rem.png",
        [RAM_NAME]: "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2a/Re_Zero_Ram.png/200px-Re_Zero_Ram.png",
        [SENKO_NAME]: "https://upload.wikimedia.org/wikipedia/en/f/fe/Sewayaki_Kitsune_no_Senko-san_volume_1_cover.jpg"
    };

    // Culori implicite - User are mov (#800080) cu text alb
    let COLORS = {
    ...
    // Setări transparență mesaje
    let messageOpacity = "solid"; // "solid", "semi-transparent", "transparent"
    ...
    function regenerateCSS() {
        ...
        // Calculăm opacitatea pentru mesaje
    ...
    function setMessageOpacity(opacity) {
        ...
        // Actualizăm butoanele de opacitate
    ...
    function handleSenkoCommand(command) {
        ...
        if (cmd === "poza ta") {
            addMessage(SENKO_NAME, selectedAvatars[SENKO_NAME], true, true);
            return true;
        }
    ...
    function createDOM() {
        ...
        // Secțiune culori fundal mesaje
        const bgSection = document.createElement('div');
        bgSection.className = 'dual-chatbot-color-section';
        const bgSectionTitle = document.createElement('h4');
        bgSectionTitle.textContent = '🎨 Culori Fundal Mesaje';
        bgSection.appendChild(bgSectionTitle);
        ...
        bgColorOptions.forEach(option => {
            ...
            bgSection.appendChild(colorOptionDiv);
        });

        colorPicker.appendChild(bgSection);

        // Secțiune culori nume
        const nameSection = document.createElement('div');
        nameSection.className = 'dual-chatbot-color-section';
        const nameSectionTitle = document.createElement('h4');
        nameSectionTitle.textContent = '👤 Culori Nume Mesaje';
        nameSection.appendChild(nameSectionTitle);
        ...
        nameColorOptions.forEach(option => {
            const colorOptionDiv = document.createElement('div');
            colorOptionDiv.className = 'dual-chatbot-color-option';
            ...
            colorOptionDiv.appendChild(colorInput);
            colorOptionDiv.appendChild(colorLabel);
            nameSection.appendChild(colorOptionDiv);
        });

        colorPicker.appendChild(nameSection);

        // Butoane transparență
        const opacitySection = document.createElement('div');
        opacitySection.className = 'dual-chatbot-color-section';
        const opacitySectionTitle = document.createElement('h4');
        opacitySectionTitle.textContent = '🎭 Transparență Fundal Mesaje';
        opacitySection.appendChild(opacitySectionTitle);
        ...
        opacityButtons.forEach(btn => {
            ...
            if (messageOpacity === btn.value) {
                button.classList.add('active');
            }
            button.addEventListener('click', () => {
                setMessageOpacity(btn.value);
            });
            opacityButtonsContainer.appendChild(button);
        });

        opacitySection.appendChild(opacityButtonsContainer);
        colorPicker.appendChild(opacitySection);

        // Secțiune alte culori
        const otherSection = document.createElement('div');
        otherSection.className = 'dual-chatbot-color-section';
        const otherSectionTitle = document.createElement('h4');
        otherSectionTitle.textContent = '🎨 Alte Culori';
        ...
        otherColorOptions.forEach(option => {
            ...
            colorOptionDiv.appendChild(colorInput);
            colorOptionDiv.appendChild(colorLabel);
            otherSection.appendChild(colorOptionDiv);
        });

        colorPicker.appendChild(otherSection);
        ...
        senders.forEach(sender => {
            ...
            if (fileSender === sender) {
                senderButton.classList.add('selected');
            }
            senderButton.addEventListener('click', () => {
                fileSender = sender;
                fileSenderSelector.querySelectorAll('button').forEach(btn => btn.classList.remove('selected'));
                senderButton.classList.add('selected');
            });
            fileSenderSelector.appendChild(senderButton);
        });
    ...
    function updateHeaderAvatars(headerLeft) {
        if (currentMode === "normal") {
            ...
            const avatar = document.createElement('img');
            avatar.src = selectedAvatars[SENKO_NAME];
            avatar.alt = SENKO_NAME;
            avatar.className = 'avatar';
            avatarContainer.appendChild(avatar);

            const nameSpan = document.createElement('span');
            nameSpan.className = 'dual-chatbot-header-name';
            nameSpan.textContent = SENKO_NAME;
            avatarContainer.appendChild(nameSpan);

            headerLeft.appendChild(avatarContainer);
        } else {
            [REM_NAME, RAM_NAME, SENKO_NAME].forEach(name => {
                const avatarContainer = document.createElement('div');
                avatarContainer.className = 'dual-chatbot-header-avatar-container';
    ...
    function renderMessages() {
        ...
        messages.forEach((msg) => {
            // Filtrează mesajele care nu aparțin modului curent
            ...
            avatarImg.src = selectedAvatars[msg.sender] || selectedAvatars[USER_NAME];
    ...
    function init() {
        ...
        elements.fileButtonHeader.addEventListener('click', () => {
            const fileSenderSelector = document.getElementById('dual-chatbot-file-sender-selector');
            fileSenderSelector.innerHTML = '';
            const senders = currentMode === "normal" ? [USER_NAME, REM_NAME, RAM_NAME] :
                           currentMode === "senko" ? [USER_NAME, SENKO_NAME] :
                           [USER_NAME, REM_NAME, RAM_NAME, SENKO_NAME];
        ...
        elements.cancelFileButton.addEventListener('click', () => {
            ...
            document.getElementById('dual-chatbot-file-input').value = '';
        });

        elements.confirmFileButton.addEventListener('click', () => {
            const fileInput = document.getElementById('dual-chatbot-file-input');
        ...
        // Ascunde panourile la click în afară
